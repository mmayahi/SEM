...
if not os.path.exists("nrg/"):
    os.makedirs("nrg/")

def parse_file(file):
    try:
        # Extract data from the output file
        stdout = file['output']['nrg.log']
...
nrg_df = pd.DataFrame(parsed_data, columns=column_names)
nrg_df.to_csv(os.path.join("nrg", "nrg_df.csv"), index=False)
# Print or further process the DataFrame as needed
print(nrg_df.head(10))