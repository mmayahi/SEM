...
if not os.path.exists("pcktloss/"):
    os.makedirs("pcktloss/")

def parse_file(file):
    try:
        # Extract data from the output file
        stdout = file['output']['pcktloss.log']
        ...
        if numbers[0] == 1:
            ...
            uplink_pcktloss = numbers[-1]
            downlink_pcktloss = 0
        elif numbers[0] == 2:
            communication_link = "Down Link"
            uplink_pcktloss = 0
            downlink_pcktloss = numbers[-2]
        elif numbers[0] == 3:
        ...
        return [communication_link, power_save_mechanism, generated_traffic,
                ...
                uplink_pcktloss, downlink_pcktloss]
...
pcktloss_df = pd.DataFrame(parsed_data, columns=column_names)
pcktloss_df.to_csv(os.path.join("pcktloss", "pcktloss_df.csv"), index=False)
# Print or further process the DataFrame as needed
print(pcktloss_df.head(10))